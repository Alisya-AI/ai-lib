...
export async function initCommand({
  ...
  if (!hasSelectionFlags) {
    ...
    const guidedPromptIO = promptIO || (await createDefaultPromptIOForTerminal());
...
async function createDefaultPromptIOForTerminal(): Promise<InitPromptIO | undefined> {
  return process.stdin.isTTY && process.stdout.isTTY
    ? (await import('./init-interactive.ts')).createDefaultPromptIO()
    : undefined;
}
